...
def read_data():
    diff = np.load("ring_event_diff.npy")
    same = np.load("ring_event_same.npy")
...
#            p3 = tf.matmul(p3, gcn_out[0])
        ...
        with tf.variable_scope('regression'):
            ...
            # p_concat = tf.concat([p1, p2], axis=1)
            # dense1 = tf.layers.dense(inputs=p_concat, units=64, activation=tf.nn.relu)
            # dense4 = tf.layers.dense(inputs=dense1, units=32, activation=tf.nn.relu)
            # dense2 = tf.layers.dense(inputs=dense4, units=16, activation=tf.nn.relu)
            # dense3 = tf.layers.dense(inputs=dense2, units=1, activation=None)
...
if __name__ == "__main__":
    ...
    adj_data = np.load("/home/LAB/penghao/nyx/HIN/LightRNN/data/adj_data.npy")
...
#                    print(v2)
                ...
                if index % 192 == 0:
                    ...
                    with open("test_acc2.txt","a+") as f:
                         f.write(str(eva_acc))
                         f.write("\n")
                    print('------------------------------------------------------Test acc: {:.4f}'.format(eva_acc))
                if acc > max_acc:
                    max_acc = acc
                    f = open('results_new2.txt', 'w')
